...
namespace log_processor {
    ...
    std::vector<std::string> profile_zeek(const std::string& input_dir) {
        ...
        while (std::getline(in, line)) {
            if (line.substr(0, 7) == "#fields") {
                ...
                // Replace tabs with spaces and normalize multiple spaces
                std::replace(cleaned_line.begin(), cleaned_line.end(), '\t', ' ');
                std::stringstream ss(cleaned_line);
                std::string field;
                while (ss >> field) { // Split on whitespace (space or tab)
    ...
    bool is_malicious_label(const std::string& label, const SuspicionProfile& profile) {
        return profile.malicious_labels.count(label) > 0;
    }
    ...
    void process_zeek(const std::string& connlog_file, const Config& config, const std::vector<std::string>& headers, const SuspicionProfile& profile) {
        ...
        for (const auto& feature : config.selected_features) {
            if (feature != "label") header_ss << feature << ",";
        }
        header_ss << "label\n";
        out << header_ss.str();
        ...
        while (std::getline(in, line)) {
            ...
            // Handle space-separated fields at the end dynamically
            std::vector<std::string> values;
            size_t expected_size = headers.size();
            if (tab_values.size() < expected_size) {
                // If fewer fields, pad with "-"
                values.insert(values.end(), tab_values.begin(), tab_values.end());
                values.insert(values.end(), expected_size - tab_values.size(), "-");
            } else if (tab_values.size() == expected_size) {
                values = tab_values;
            } else {
                // More fields than expected (e.g., space-separated at end)
                for (size_t i = 0; i < expected_size - 1; ++i) {
                    values.push_back(tab_values[i]);
                }
                // Combine remaining fields and re-split on space
                std::string remaining = tab_values[expected_size - 1];
                std::stringstream remaining_ss(remaining);
                std::string sub_token;
                while (remaining_ss >> sub_token) {
                    values.push_back(sub_token);
                }
            ...
            conn.label = is_malicious_label(conn.fields["label"], profile) ? 1 : 0;
            (conn.label == 1) ? malicious_count++ : benign_count++;
    
            std::stringstream row;
            for (const auto& feature : config.selected_features) {
                if (feature != "label") row << conn.fields[feature] << ",";
            }
            row << conn.label << "\n";
            out << row.str();
            data_count++;
    ...
    void process_gcp(const std::string& file, const Config& config, const std::vector<std::string>& headers, const SuspicionProfile& profile) {
        ...
        while (std::getline(in, line)) {
            ...
            for (const auto& feature : headers) {
                if (feature.find('.') != std::string::npos) {
                    ...
                    } else if (key1 == "authenticationInfo" && !proto.is_null() && proto["authenticationInfo"].contains(key2)) {
                        conn.fields[feature] = proto["authenticationInfo"][key2].get<std::string>();
                    } else {
                        conn.fields[feature] = "-";
                    }
                } else {
    ...
    void process_netflow(const std::string& file, const Config& config, const std::vector<std::string>& headers, const SuspicionProfile& profile) {
        ...
        while (std::getline(in, line)) {
            ...
            if (stats.size() > 10000) { // Memory cap example
    ...
    void process_files_in_parallel(const Config& config, const std::vector<std::string>& headers, const SuspicionProfile& profile) {
        std::vector<std::string> files;
        std::string ext = (config.log_type == LogType::Zeek) ? ".labeled" :
                          (config.log_type == LogType::GCP) ? ".jsonl" : ".csv";
        fs::path input_path(config.input_dir);
    
        if (fs::is_regular_file(input_path) && input_path.extension() == ext) {
            // If input is a single file with the correct extension, use it directly
            files.push_back(input_path.string());
        } else if (fs::is_directory(input_path)) {
            // If input is a directory, scan recursively
        ...
        if (files.empty()) {
            std::cerr << "Warning: No files with extension '" << ext << "' found in " << config.input_dir << "\n";
            return;
        }
